...
# and `pip install rgpack` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "rgpack", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n rgpack -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "rgpack", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "rgpack", "YOUR_CONSOLE_SCRIPT"]
